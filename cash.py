...
# Function to save income data to CSV
def save_income_to_csv(income, date, deposit):
    df = pd.DataFrame({
...
# Function to save sector data to CSV
def save_sector_to_csv(sector, amount, date):
    df = pd.DataFrame({
        'Sector': [sector],
        'Amount': [amount],
        'Date': [date]
...
# Function to load income and expense data
def load_data():
    ...
    if os.path.exists(income_csv_file):
        try:
            ...
            income_data.columns = income_data.columns.str.strip()  # remove extra spaces
            if 'Date' in income_data.columns:
                ...
                st.error("'Date' column not found in income_data.csv")
    ...
    if os.path.exists(sector_csv_file):
        try:
            ...
            if 'Date' in sector_data.columns:
                ...
                st.error("'Date' column not found in sector_data.csv")
